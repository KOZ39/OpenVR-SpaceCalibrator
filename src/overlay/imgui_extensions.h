...
inline bool IconButton(const char* icon, const char* label, const ImVec2& size_arg = ImVec2(0, 0), ImGuiButtonFlags flags = 0)
{
    ...
    if (!label || label[0] == '\0')
        return ImGui::ButtonEx(icon, size_arg, flags);
...
bool InformationButton(const char* icon, const char* title, const char* desc, ImTextureID previewTex, ImVec2 previewSize);
