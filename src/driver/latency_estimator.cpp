...
namespace spacecal {
    ...
    double LatencyEstimator::get_latency(vr::TrackedDeviceIndex_t refIndex, vr::TrackedDeviceIndex_t targetIndex) {
        ...
        if (entry.referenceFrameCount.load(std::memory_order_acquire) == refFrames &&
            entry.targetFrameCount.load(std::memory_order_acquire) == targetFrames) {
        ...
        if (!hadEstimate) {
            ...
            bool candidateIsMeaningfullyBetter =
                (differenceAtCurrent == DBL_MAX) || (differenceAtCandidate < differenceAtCurrent - k_MIN_IMPROVEMENT_TO_SWITCH);
